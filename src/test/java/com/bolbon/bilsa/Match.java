...
public class Match {
	...
	public static String simulationSimple(Team home, Team away) {
		/*
		 * int calcularResultadodelPartido(int loc[90][2], float varloc){
		 * int goles=0; //variable para retornar la cantidad de goles marcados 
		 * 	for (int i=0;i<90;i++){ //inicializar goles por minuto 
		 * 		if ((float)loc[i][0]<varloc){//si el random es menor al determinante 
		 * 			loc[i][1]=1; //es gol 
		 * 			goles=goles+1;//acumulador de goles 
		 * return goles; 
		 */
		...
		int resultHome = sumaGoles(goalsHome);
		int resultAway = sumaGoles(goalsAway);
		
		return (home.getRating()-away.getRating() +":"+ home.getName()+ " "+ resultHome + " - "+ resultAway + " "+away.getName());
	}
	...
	/*
	 * void numeroRandomaMatriz(int tam, int matriz[90][2]){ 
	 * for (int i = 0;i<tam;i++){ 
	 * 		matriz[i][0]=rand()%10000; //le asigno valor a la variable random
	 * 		matriz[i][1]=0; //inicializo los goles en 0 } 
	 * }
	 */
	...
	public static int sumaGoles(int[] matrixGoles) {
		int suma=0;
		for (int i = 0; i<matrixGoles.length; i++) suma+=matrixGoles[i];
		return suma;
	}
}
